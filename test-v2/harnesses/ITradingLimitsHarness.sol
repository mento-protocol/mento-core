...
interface ITradingLimitsHarness {
  ...
  function validate(Config calldata config) external view;

  function verify(State calldata state, Config calldata config) external view;

  function reset(State calldata state, Config calldata config) external view returns (State memory);
