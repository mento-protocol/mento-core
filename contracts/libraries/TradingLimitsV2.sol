...
library TradingLimits {
   ...
   */
  function validate(IBroker.Config memory self) internal pure {
   ...
   */
  function verify(IBroker.State memory self, IBroker.Config memory config) internal pure {
   ...
   */
  function reset(IBroker.State memory self, IBroker.Config memory config) internal pure returns (IBroker.State memory) {
   ...
   */
  function update(
    IBroker.State memory self,
    IBroker.Config memory config,
    int256 _deltaFlow,
    uint8 decimals
  ) internal view returns (IBroker.State memory) {
