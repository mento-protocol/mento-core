...
library TradingLimits {
  ...
  function validate(ITradingLimits.Config memory self) internal pure {
    ...
    require(
      self.flags & (L1 | LG) != 6 || self.limit1Out < self.limitGlobal,
      "limitGlobal must be greater than limit1Out"
    );
    require(
      self.flags & (L0 | LG) != 5 || self.limit0In < self.limitGlobal,
      "limitGlobal must be greater than limit0In"
    );
    require(
      self.flags & (L0 | LG) != 5 || self.limit0Out < self.limitGlobal,
      "limitGlobal must be greater than limit0Out"
    );
  }
