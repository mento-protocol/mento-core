...
contract BancorExchangeProvider is IExchangeProvider, IBancorExchangeProvider, BancorFormula, OwnableUpgradeable {
  ...
  function createExchange(PoolExchange calldata _exchange) external onlyOwner returns (bytes32 exchangeId) {
    ...
    exchangeId = keccak256(
      abi.encodePacked(IERC20Metadata(exchange.reserveAsset).symbol(), IERC20Metadata(exchange.tokenAddress).symbol())
    );
    require(exchanges[exchangeId].reserveAsset == address(0), "Exchange already exists");

    uint256 reserveAssetDecimals = IERC20Metadata(exchange.reserveAsset).decimals();
    uint256 tokenDecimals = IERC20Metadata(exchange.tokenAddress).decimals();
