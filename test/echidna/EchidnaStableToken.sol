...
contract EchidnaStableToken {    
    ...
    constructor() public{
        ...
        stableToken.initialize("Celo Dollar", "cUSD", 18, address(registry), 1e24, 1 weeks, new address[](0), new uint256[](0),"Exchange");     
    }
    ...
    function totalSupplyConsistantERC20Properties(uint120 user1Amount, uint120 user2Amount, uint120 user3Amount ) public{
        address user1 = address(0xfffffff);
        address user2 = address(0xffffff);
        address user3 = address(0xfffff); 
        assert( stableToken.balanceOf(user1) + stableToken.balanceOf(user2) + stableToken.balanceOf(user3) == stableToken.totalSupply());
