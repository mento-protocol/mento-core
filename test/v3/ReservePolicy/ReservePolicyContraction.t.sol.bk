...
contract ReservePolicyContractionTest is ReservePolicyBaseTest {
  ...
  function test_determineAction_whenPoolPriceBelowOracleWithZeroIncentive_shouldReturnCorrectAmounts() public view {
    ...
    // Formula: X = (ON * RD - OD * RN) / (OD * (2 - i))
    // X = (1e18 * 200e18 - 1e18 * 100e18) / (1e18 * 2)
    // X = 100e18 / 2 = 50e18 (token1 to add)
    ...
    // Y = X * OD/ON = 50e18 * 1e18/1e18 = 50e18 (token0 to remove)
  ...
  function test_determineAction_whenPoolPriceBelowOracleWithMaxIncentive_shouldReturnCorrectAmounts() public view {
    ...
    // X = 100e18 / 1 = 100e18 (token1 to add)
    // In contraction: collateral flows in via inputAmount
    assertEq(action.inputAmount, 100e18, "Should calculate correct collateral input amount");
    // Y = X * OD/ON = 100e18 * 1e18/1e18 = 100e18 (token0 to remove)
  ...
  function test_formulaValidation_whenPPLessThanOP_shouldFollowExactFormula() public view {
    ...
    // X = (1000e18 - 100e18) / 2 = 900e18 / 2 = 450e18
    uint256 expectedX = 450e18;
    assertEq(action.inputAmount, expectedX, "X calculation should match formula (collateral input)");
    // For PP < OP, token1 flows in via inputAmount and token0 flows out
    
    // Y = X * OD/ON = 450e18 * 1e18/2e18 = 225e18
    uint256 expectedY = 225e18;
    assertEq(action.amount0Out, expectedY, "Y should equal X * OD/ON");
  }

  function test_YRelationship_shouldAlwaysHoldForContraction() public view {
    // Test Y = X * OD/ON relationship for contraction (PP < OP)
    ...
    for (uint i = 0; i < incentives.length; i++) {
      ...
      if (shouldAct && action.inputAmount > 0) {
        // Y/X should equal OD/ON (Y is amount0Out, X is inputAmount) within precision limits
        uint256 calculatedRatio = (action.amount0Out * ctx.prices.oracleNum) / action.inputAmount;
