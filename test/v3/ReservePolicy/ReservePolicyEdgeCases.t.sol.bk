...
contract ReservePolicyEdgeCasesTest is ReservePolicyBaseTest {
  ...
  function test_edgeCase_whenIncentiveAt20000_shouldReturnZero() public view {
    // NOTE: In the new architecture, when incentiveBps is 20000, the denominator becomes 0
    // This should cause division by zero, which will revert
    // This test expects the function NOT to revert with shouldAct=false, but that's not how it works
    // Commenting out this test as division by zero would revert the transaction
    // The validation of incentive should happen at a higher level
    ...
    // // NOTE: In the new architecture, this will cause a division by zero and revert
    // // The old test expected shouldAct=false, but the new implementation doesn't check for this
