 ...
 */
abstract contract PullOracleForkBase is Test {
...
/**
 * @title PullOracleRelayerNegativePath
 ...
 */
contract PullOracleRelayerNegativePath is PullOracleForkBase {
  ...
  function _singleLeg(bytes32 feedId) internal pure returns (IPullOracleRelayer.OracleLeg[] memory legs) {
    legs = new IPullOracleRelayer.OracleLeg[](1);
    legs[0] = IPullOracleRelayer.OracleLeg(feedId, false);
  }

  function _deployRelayer(address verifier) internal returns (PullOracleRelayerV1 relayer) {
    // Wrap the (mock) VerifierProxy in the Chainlink adapter — the relayer only sees IPullOracleAdapter.
    address adapterAddr = address(new ChainlinkDataStreamsAdapter(verifier));
    relayer = new PullOracleRelayerV1(
      rateFeedId,
      "CELO/USD",
  ...
  function test_strictMock_acceptsAuthenticPayload() public {
    ...
    PullOracleRelayerV1 relayer = _deployRelayer(address(verifier));
    ...
    relayer.relay(abi.encode(_wrap(signed)));
  ...
  function test_tamperedSignatureByte_reverts() public {
    ...
    relayer.relay(abi.encode(_wrap(tampered)));
  }
  ...
  function test_expiredReport_reverts() public {
    ...
    relayer.relay(abi.encode(_wrap(report)));
  }
